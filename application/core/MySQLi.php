...
class MySQLi extends \MySQLi
{
    ...
    {
        parent::__construct(dbconf::HOST, dbconf::USER, dbconf::PASSWORD, dbconf::DATABASE);
    ...
    {
        $stmt = $this->prepare("SELECT * FROM `threads` ORDER BY date_post DESC ");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        if ($stmt->errno != 0) throw new DbException($stmt->error, $stmt->errno);
    ...
    {
        $stmt = $this->prepare("select login from users u left join threads t on u.id = t.author_id where t.id = ?");
    ...
    public function getAllThreadsByAuthorName($author_name)
    {
        $stmt = $this->prepare("select title, t.id from users u left join threads t on u.id = t.author_id where u.login = ?");
    ...
    {
        $stmt = $this->prepare("INSERT INTO threads (title, text, img, author_id) VALUES (?,?,?,?)");
        $stmt->bind_param("sssi", $title, $text, $img, $_COOKIE['id']);
        $stmt->execute();
        if ($stmt->errno != 0) throw new DbException($stmt->error, $stmt->errno);
